...
- **背牌减速**：每张背包牌 **−3%** 移速，最低 **50%**（`shared/config/movement.ts`）
- **大厅**：主路径为 **单机游戏**（本地 3 机器人）；**局域对战暂未开启**（联机代码仍保留）
- **单机完整玩法**（无 WebSocket）：运牌、偷家、电网触电、你铲 / 机器人铲、2 分钟 / 50 张、UNO 时刻（`OfflineBotSystem` + 本地权威）
...
- **UNO 时刻**：某玩家老家分首次进入「距胜利还差 ≤10 张」时：该玩家**头顶 + 老家**显示 **UNO!**，全房顶部横幅「XXXX玩家还差N张牌就获得胜利，UNO时刻！」（`uno_moment`）
- **老家电网**：主人站在自己老家平台内 → 四周全员可见电网；**非主人踩入即触电死亡**（全掉落背包+手持），**5s** 后自家重生；有电网时不可偷该家
- **回合胜负**：2 分钟；先把牌送达老家 **50 张**者胜；超时则老家张数最多者胜（`shared/config/match.ts`）
...
| `src/entities/Player.ts` | 本地/远端豆人（胶囊 + **程序化小手小脚**）+ 手持棒/Skip + 背牌；跑/铲/击飞/眩晕四肢假动画 |
...
- 时长 `MATCH_DURATION_MS`（默认 2 分钟）、达标 `MATCH_WIN_SCORE`（默认 50，仅计老家）  
- `match_start` 带 `endsAt` / `winScore`；结束广播 `match_end` 并回大厅  
- HUD 倒计时 + 结算弹窗  
- **UNO 时刻**：送达后分数首次满足 `score >= WIN - MATCH_UNO_REMAINING`（默认还差 **10** 张）且未获胜 → 每人每局一次；该家 3D **UNO!** + 全房顶栏文案  
